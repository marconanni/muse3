...
//	variabili gestione thread di parsing stream RTP
	...
	//*******************************************
	private boolean pauseParser = false;
	private Object parsingSync = new Object();
//*******************************************
...
//metodo costruttore che inizializza la classe thread col parser e la dimensione del CirclarBuffer
	public ParserThreadEV(Parser parser, int buffersize, IClientView view, int sogliaInferiore, int sogliaSuperiore)
	{
		...
		for (int i=0; i<numberOfTracks; i++) bufferSet[i]=new EventCircularBuffer(buffersize,view, sogliaInferiore, sogliaSuperiore);
    }
	...
	public void parsing()
	{
		...
		try
		{
			do
			{
				...
				if(pauseParser)
				{
					System.out.println("------------------------ParserThreadEV parsing method: sono in pausa");
					synchronized(parsingSync)
					{
						System.out.println("+++++++++++++++++++++++ParserThreadEV parsing method: mi sto sospendendo");
						try{ parsingSync.wait(); }
						catch(InterruptedException e){ System.err.println(e); }
					}
					System.out.println("*****************************ParserThreadEV parsing method: riavvio il thread del parser");
				}
				//********Luca********************************************************************************
...
//metodo che restituisce il tempo medio (mediato su 10 arrivi) impiegato ad estrarre l'ultimo frame giunto, in caso tale tempo non sia inferiore a 10 secondi (ad esempio primo frame estratto per cui non si possiede il tempo di estrazione del precedente o altre attese che altererebbero la valutazione) viene restituito -1
	...
	/**
	 * METODI AGGIUNTI DAL GRUPPO MUSE 4
	 */
	public void suspend()
	{
		pauseParser=true;
		if(pauseParser)System.out.println("--------------------------------------->ParserThreadEV suspend method: richiesta sospensione del thread del parser");
		else System.out.println("------------> ParserThreadEV suspend method: NESSUNA richiesta sospensione x il thread del parser");
		
	}
	
	public synchronized void restart()
	{
		if(!pauseParser) return;
		pauseParser=false;
		synchronized(parsingSync){ parsingSync.notify(); }
		System.out.println("ParserThreadEV restart method: riavvio del thread del parser");
	}

	public EventCircularBuffer[] getBufferSet() {
		return bufferSet;
	}

	public void setBufferSet(EventCircularBuffer[] bufferSet) {
		this.bufferSet = bufferSet;
	}

	public void setRTPParser(RTPParser plugin) {
		this.plugin = plugin;
	}

	public long getFrameChange() {
		return frameChange;
	}

	public void setFrameChange(long frameChange) {
