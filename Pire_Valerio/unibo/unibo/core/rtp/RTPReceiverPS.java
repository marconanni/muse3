	...
	public RTPReceiverPS(int localPort, InetAddress local, InetAddress source) throws IOException
		{
			...
			try
			{
				...
				//SessionAddress serverAddr=new SessionAddress(InetAddress.getLocalHost(),20000);
				SessionAddress serverAddr=new SessionAddress(source,20000);//se mi collego ad un normale relay devo cambiare
...
//se e' prevista la possibilita' di chiamare piu' volte questo metodo assegnando vari trasmettitori da cui ricevere allora sarebbe il caso di spostare nel costruttore l'inizializzazione di dataSynch
	public void setSender(InetAddress host, int port) throws IOException
	{
		...
		//System.err.println("SENDER: "+ host.getHostName()+" PORT: "+port);
		System.err.println("SENDER: "+host.toString()+" PORT: "+port);
...
//Verificare il motivo di questa situazione ed inserire il codice appropriato per gestirla.
	...
	public void removeTargets(){
		rtpManager.removeTargets("Rimozione Forzata delle sessioni in atto");
		System.out.println("RTPReceiverPS.removeTargets: rimossi tutti i partecipanti ");
		receivedDataSource = null;
	}


	public void addSource(InetAddress newIp, int newPort)throws IOException{
		SessionAddress address = new SessionAddress(newIp, newPort);
		try {
			rtpManager.addTarget(address);
			System.out.println("RTPReceiverPS.addTarget: partecipanti aggiunti");

		} catch (InvalidSessionAddressException e) {
			throw new IOException("Adding target failed:\n"+e);
		} catch (UnknownHostException ie) { 
			throw new IOException("Destination Host Unknown:\n"+ie); 
		}
