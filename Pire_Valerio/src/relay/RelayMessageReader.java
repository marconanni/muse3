...
public class RelayMessageReader {
	...
	private int portClientSessionIn;
	...
	private int bigbossStreamingInputPort;
	private int relayControlPort;
	private int relayStreamingInputPort;
	private int clientControlPort;
	private int clientStreamingInputPort;
	 ...
	 * */
	 private void readMessage(){
		...
		if(code==Parameters.FORWARD_REQ_LIST){
			clientAddress=st.nextToken();
		}
		
		if (code==Parameters.LIST_RESPONSE){
			listaFile=st.nextToken();
			clientAddress=st.nextToken();
			portClientSessionIn=Integer.parseInt(st.nextToken());
		}
		if(code==Parameters.FORWARD_LIST_RESPONSE){
		...
		if(code==Parameters.FORWARD_REQ_FILE){
			...
			bigbossStreamingInputPort=Integer.parseInt(st.nextToken());
			relayAddress=st.nextToken();
			relayControlPort=Integer.parseInt(st.nextToken());
			relayStreamingInputPort=Integer.parseInt(st.nextToken());
			clientAddress=st.nextToken();
			clientStreamingInputPort=Integer.parseInt(st.nextToken());
		}
		/*
		 * 
		...
		*/
		...
		if(code == Parameters.ACK_REQUEST_FILE)
		{
			clientAddress=st.nextToken();
			portClientSessionIn=Integer.parseInt(st.nextToken());
			portStreamingClient=Integer.parseInt(st.nextToken());
	...
	public int getBigbossControlPort() {
		return bigbossControlPort;
	}
	public int getRelayControlPort() {
		return relayControlPort;
	}
	public int getClientStreamingInputPort() {
		return clientStreamingInputPort;
	}
	public int getRelayStreamingInputPort() {
		return relayStreamingInputPort;
	}
