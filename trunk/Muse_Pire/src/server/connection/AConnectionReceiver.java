...
/*class TestConnectionReceiver{
	...
	public static void main (String args[]){
		...
		cr = new AConnectionReceiver(observer, localhost.toString(),6000);
		...
		byte[] buffer = {1,2,3,4};
		byte[] buffer2 = {2,3,5,6};
		DatagramPacket dp = new DatagramPacket(buffer,buffer.length,localhost,6000);
		DatagramPacket dp1 = new DatagramPacket(buffer2,buffer2.length,localhost,6000);
		...
		//ds.close();
		System.out.print("Mandato il primo pacchetto");
		try {
			//ds2 = new DatagramSocket(5000, localhost);
			ds.send(dp1);
		} catch (SocketException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
