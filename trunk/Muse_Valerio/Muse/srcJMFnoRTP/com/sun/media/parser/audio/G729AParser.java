...
public class G729AParser extends BasicPullParser {
    private static final int CODEFRAMESIZE = 10;// code frame size in bytes
    private static final int DATAFRAMESIZE = 160;// data frame size in bytes
    private static final int FRAMERATE = 100;	// frames per second
    ...
    private static ContentDescriptor[] supportedFormat = new ContentDescriptor[] {new ContentDescriptor("audio.g729a"), new ContentDescriptor("audio.g729")};
	...
	tracks[0] = new G729ATrack((AudioFormat) format,
				/*enabled=*/ true,
				new Time(0),
				numBuffers,
				bufferSize,
	...
	format = new AudioFormat(AudioFormat.G729A,
				 8000,
				 16,
				 1,
...
// 	    newPos = maxLocation;
	...
	return "G.729A Parser";
    }

    class G729ATrack extends BasicTrack {
	private double sampleRate;
	private float timePerFrame;
	private SettableTime frameToTime = new SettableTime();

	G729ATrack(AudioFormat format, boolean enabled, Time startTime,
	       int numBuffers, int bufferSize,
	       long minLocation, long maxLocation) {
	    super(G729AParser.this, 
		  format, enabled, G729AParser.this.duration,
		  startTime, numBuffers, bufferSize,
		  G729AParser.this.stream, minLocation, maxLocation);
