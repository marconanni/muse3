...
// Source File Name:   BufferControlImpl.java
    ...
    implements BufferControl
{
    ...
    protected void initBufferControl(Format f)
    {
        if(f instanceof AudioFormat)
        {
            ...
            currThreshold = currThreshold != 0x7fffffffL ? currThreshold : defThreshold;
        }
    ...
    public long setBufferLength(long time)
    {
        ...
        if(time <= 0L || time == defBuffer)
            currBuffer = defBuffer;
        else
            currBuffer = time;
        for(int i = 0; i < sourcestreamlist.size(); i++)
            ((RTPSourceStream)sourcestreamlist.elementAt(i)).updateBuffer(currBuffer);
    ...
    public long setMinimumThreshold(long t)
    {
        ...
        if(t < 0L)
            currThreshold = 0L;
        for(int i = 0; i < sourcestreamlist.size(); i++)
            ((RTPSourceStream)sourcestreamlist.elementAt(i)).updateThreshold(currThreshold);
