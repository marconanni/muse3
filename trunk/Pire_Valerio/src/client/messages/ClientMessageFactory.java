...
//VALERIO SANDRI
	...
	public static DatagramPacket buildRequestList(int sequenceNumber, InetAddress relayAddress, int relayControlPort) throws IOException{
		ByteArrayOutputStream boStream = new ByteArrayOutputStream();
		DataOutputStream doStream = new DataOutputStream(boStream);
		String content= sequenceNumber+"_"+MessageCodeConfiguration.REQUEST_LIST;
		doStream.writeUTF(content);
		doStream.flush();
		byte[] data = boStream.toByteArray();
	...
	public static DatagramPacket buildStartTX(int sequenceNumber, InetAddress addr, int port) throws IOException{
		System.err.println("Inviato START_TX"+addr);
		ByteArrayOutputStream boStream = new ByteArrayOutputStream();
		DataOutputStream doStream = new DataOutputStream(boStream);
	...
	public static DatagramPacket buildStopTX(int sequenceNumber, InetAddress addr, int port) throws IOException{
		...
		System.err.println("-------------HO INVIATO UNO STOP TX-------");
		ByteArrayOutputStream boStream = new ByteArrayOutputStream();
		DataOutputStream doStream = new DataOutputStream(boStream);
