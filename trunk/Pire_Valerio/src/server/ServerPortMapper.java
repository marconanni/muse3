...
public class ServerPortMapper {
	...
	private boolean[] rangePortInOutControlStreamingServer = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 
	
	/*range di porte assegnabili ad uno STREAMINGSERVER per l'invio del flusso RTP al PROXY*/
	private boolean[] rangePortOutRTPStreamingServer = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS];
	...
	private ServerPortMapper()
	{
		...
		portInSession = PortConfiguration.SERVER_SESSION_PORT_IN;
		
		try {
			localAddress = InetAddress.getByName(NetConfiguration.SERVER_ADDRESS);
		...
		if(typePort.equals("Control"))
		{
			for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
				if(rangePortInOutControlStreamingServer[i]){
					...
					return i + PortConfiguration.STREAMINGSERVER_INITIAL_PORT_IN_OUT_CONTROL;
				}
			}
		}
		if(typePort.equals("Stream"))
		{
			for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
				if(rangePortOutRTPStreamingServer[i]){
					...
					return i + PortConfiguration.STREAMINGSERVER_INITIAL_PORT_OUT_RTP;
				}
			}
		}
		return -1;
