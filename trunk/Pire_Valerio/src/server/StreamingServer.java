...
public class StreamingServer implements ControllerListener, Observer{
	...
	public StreamingServer(String filename, String proxyAddress, int proxyCtrlPort, int proxyReceiveStream, ServerSessionManager sessionManager, DebugConsole consolle) throws IOException, NoDataSourceException
	{
		...
		//this.filename = "file://home/pier/mp3/prova.mp3";
		...
		try{
			...
			consolle.debugMessage(3,"STREAMING_SERVER: Impossibile trovare il file specificato: "+filename);
			throw x;
		}
	...
	public void update(Observable receiver, Object msg) {
		...
		try {
			...
			consolle.debugMessage(3,"STREAMING_SERVER_UPDATE: impossibile leggere il pacchetto arrivato...");
		}

		if(ServerMessageReader.getCode() == MessageCodeConfiguration.START_TX && !this.status.equals("Trasmitting"))
		{
			consolle.debugMessage(2,"STREAMING_SERVER_UPDATE: Arrivata richista di START_TX");
		...
		{
			consolle.debugMessage(2,"STREAMING_SERVER_UPDATE: Arrivata richista di STOP_TX");
	...
	public int getTransmissionPort(){ return this.transmissionPort;}
	public int getTransmissionControlPort(){return this.transmissionCtrlPort;}
	...
	public void controllerUpdate(ControllerEvent ce) {
		...
		if(ce instanceof EndOfMediaEvent){
			...
			consolle.debugMessage(1,"STREAMING_SERVER_CONTROLLER_UPDATE: Media Finito");
			if(processor!=null){
				processor.stop();
				processor.close();
				processor = null;
