...
public class RelayPortMapper {
	...
	private boolean[] rangePortInRTPProxy = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 

	/*range di porte assegnabili ad un PROXY per l'invio del flusso RTP al CLIENT*/
	private boolean[] rangePortOutRTPProxy = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 

	/*range di porte assegnabili ad un PROXY per ricevere messaggi START_TX e STOP_TX dal CLIENT*/
	private boolean[] rangeAdHocPortInControlProxy = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 

	/*range di porte assegnabili ad un PROXY per inviare il messaggio di LEAVE alCLIENT*/
	private boolean[] rangeAdHocPortOutControlProxy = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 
	...
	private boolean[] rangeManagedPortInOutControlProxy = new boolean[SessionConfiguration.RANGE_ACTIVE_SESSIONS]; 
	...
	{
 		for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
 		...
 		portInAdHocSession = PortConfiguration.RELAY_SESSION_AD_HOC_PORT_IN;
//		portInAdHocSession = Parameters.BIG_BOSS_SESSION_AD_HOC_PORT_IN;
		portOutAdHocSession = PortConfiguration.RELAY_SESSION_AD_HOC_PORT_OUT;
		portOutManagedSession = PortConfiguration.RELAY_SESSION_MANAGED_PORT_OUT;
...
//	public InetAddress getLocalManagedBCASTAddress(){return localManagedBCASTAdress;}
	...
	public void setRangeAdHocPortInControlProxy(
		...
		this.rangeAdHocPortInControlProxy[rangeAdHocPortInControlProxy - PortConfiguration.PROXY_INITIAL_AD_HOC_PORT_IN_CONTROL] = false;
	}
	...
	public void setRangeAdHocPortOutControlProxy(
		...
		this.rangeAdHocPortOutControlProxy[rangeAdHocPortOutControlProxy - PortConfiguration.PROXY_INITIAL_AD_HOC_PORT_OUT_CONTROL] = false;
	}
	...
	public void setRangeManagedPortInOutControlProxy(
		...
		this.rangeManagedPortInOutControlProxy[rangeManagedPortInOutControlProxy - PortConfiguration.PROXY_INITIAL_MANAGED_PORT_IN_OUT_CONTROL] = false;
	}
		...
		if(typePort.equals("ControlAdHocIn"))
		{
			for(int i = 0;i < 1000;i++){
				if(rangeAdHocPortInControlProxy[i]){
					...
					return i + PortConfiguration.PROXY_INITIAL_AD_HOC_PORT_IN_CONTROL;
				}
			}
		}
		...
		if(typePort.equals("ControlAdHocOut"))
		{
			for(int i = 0;i < 1000;i++){
				if(rangeAdHocPortOutControlProxy[i]){
					...
					return i + PortConfiguration.PROXY_INITIAL_AD_HOC_PORT_OUT_CONTROL;
				}
			}
		}
		...
		if(typePort.equals("ControlManagedOut"))
		{
			for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
				if(rangeManagedPortInOutControlProxy[i]){
					...
					return i + PortConfiguration.PROXY_INITIAL_MANAGED_PORT_IN_OUT_CONTROL;
				}
			}
		}
		...
		if(typePort.equals("StreamIn"))
		{
			for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
				if(rangePortInRTPProxy[i]){
					...
					return i + PortConfiguration.PROXY_INITIAL_PORT_IN_RTP;
				}
			}
		}
		...
		if(typePort.equals("StreamOut"))
		{
			for(int i = 0;i < SessionConfiguration.RANGE_ACTIVE_SESSIONS;i++){
				if(rangePortOutRTPProxy[i]){
					...
					return i + PortConfiguration.PROXY_INITIAL_PORT_OUT_RTP;
				}
			}
		}
